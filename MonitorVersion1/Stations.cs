...
namespace MonitorVersion1
{
    public partial class Stations : Form
    {
        ...
        private void Stations_Load(object sender, EventArgs e)
        {
            ...
            runCheckAll();
            showNumberStation();
            TongKhi = nDL + nMTuan + nCT + nTC1 + nGiayAH + nNhomCT + nNhomMD + nMTien + nETC + nNM2 + _HV + _PT + _SS + _TA + _VN + _TS + _TC3 + _HL ;
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            TongKhi = nDL + nMTuan + nCT + nTC1 + nGiayAH + nNhomCT + nNhomMD + nMTien + nETC + _HV + _PT + _SS + _TA + _VN + _TS; //TC3, HL, NM2
            TongNuoc = nSabeco + nAriyana;
            TongNuocNgam = _Acecook;
        }
        ...
        private void runCheck(bool isChecked, Panel panel)
        {
            if (isChecked)
            {
                panel.BackColor = Color.FromArgb(44, 202, 115);//Connetion - Xanh
            }
            else
            {
                panel.BackColor = Color.FromArgb(229, 76, 60);//Disconnetion - Do
            }
        }
